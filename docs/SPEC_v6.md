...
**Status:** Draft for Codex build — full end-to-end
...
1. **Operator registry** — tách 5 op (eq/in/prefix/contains/between) khỏi hardcode, đưa vào `operators.yaml` với template SQL. Thêm 1 op mới `suffix` (yêu cầu manager). Excel dropdown hiển thị tiếng Việt thay code. Thêm operator tương lai = 1 dòng YAML, zero code change.
...
3. **UX cải thiện** — cột `Digits` cho prefix/suffix (constraint length), value dropdown khi cardinality thấp (auto-scan DB), cho phép 1 request bao cả Export + Import (output 2 sheet), track requester từ metadata + log CSV.
...
| Excel operator UX | code text `eq/in/prefix/contains/between` | dropdown VN "Bằng / Trong danh sách / …" | +readability |
| Filter columns trong Cột Export | 3 (Toán tử, Giá trị, Lấy về?) | 4 (thêm Digits) | +1 cột conditional |
...
3. Excel template v6 — dropdown VN, cột `Digits`, conditional visibility Digits, dropdown value từ sheet `Values`
...
operators:
  ...
  prefix:
    ...
    sql_single_with_digits: "{col} LIKE {val_prefix} AND LENGTH({col}) = {digits}"
    sql_multi_with_digits: "({col} LIKE ANY({vals_prefix})) AND LENGTH({col}) = {digits}"
    multi_value: true
    arity: null
  ...
  suffix:
    ...
    sql_single_with_digits: "{col} LIKE {val_suffix} AND LENGTH({col}) = {digits}"
    sql_multi_with_digits: "({col} LIKE ANY({vals_suffix})) AND LENGTH({col}) = {digits}"
    multi_value: true
    arity: null
    supports_digits: true

# Thứ tự trong Excel dropdown (đảo được — display order tách khỏi định nghĩa)
display_order:
  - eq
  - in
  - between
...
- Nếu `supports_digits: true` → phải có `sql_*_with_digits` variant
...
class OperatorBuilder:
    ...
    def build_where(self, col: str, op: str, val: str, digits: int | None = None):
        ...
        # Chọn template dựa trên (single/multi) × (with/without digits)
        multi = len(parts) > 1 or spec.get("arity") not in (None, 1)
        with_digits = digits is not None and spec["supports_digits"]

        key = ("sql_multi" if multi else "sql_single") + ("_with_digits" if with_digits else "")
        template_str = spec.get(key) or spec[key.replace("_with_digits", "")]
    ...
    def display_labels(self) -> list[tuple[str, str]]:
        """Trả list (key, display) theo display_order — dùng cho Excel dropdown."""
...
**Header row (row 1):** `Cột` | `Toán tử` | `Giá trị` | `Digits` | `Lấy về?`

**Data rows:** N = số cột trong `datasets.<name>.columns`.

| Cột | Toán tử | Giá trị | Digits | Lấy về? |
|---|---|---|---|---|
| so_to_khai | dropdown VN | text/dropdown | conditional | dropdown |
| ma_so_hang_hoa | dropdown VN | text | 4 (nếu prefix) | YES |

**Data validation cột `Toán tử` (B2:B<N+1>):**

- Dropdown list = `display_order` từ `operators.yaml`
- Hiển thị VN: `Bằng`, `Trong danh sách`, `Trong khoảng`, `Bắt đầu bằng`, `Chứa`, `Kết thúc bằng`
- Allow blank

**Data validation cột `Giá trị` (C2:C<N+1>):**

- Nếu cột có entry trong `datasets.<name>.cardinality_cache` và giá trị ≤ threshold → dropdown list từ sheet `Values` (named range `{col}_values`)
...
**Data validation cột `Digits` (D2:D<N+1>):**

- Formula: `=OR($B2="Bắt đầu bằng", $B2="Kết thúc bằng")`
- Input type: whole number, min=1, max=50
- Nếu operator ≠ prefix/suffix → error "Digits chỉ dùng cho Bắt đầu bằng / Kết thúc bằng"
- Allow blank

**Conditional formatting cột `Digits`:**

- Rule: nếu `$B2` không phải "Bắt đầu bằng" và không phải "Kết thúc bằng" → background xám `#E7E6E6`, font xám nhạt `#A6A6A6`
- Visual "cell này không active"

**Conditional formatting row anchor (giữ v5):**

- Nếu `$B2` không rỗng → highlight row `A2:E2` bg `#FFF2CC`

**Column widths:**

- A: 35, B: 22 (VN dài hơn code), C: 30, D: 10, E: 12
...
> Chỉ dùng với `Bắt đầu bằng` / `Kết thúc bằng`. Điền số nguyên = độ dài chuỗi kỳ vọng.
> Ví dụ HS Code `Bắt đầu bằng 84` + `Digits 4` → chỉ khớp mã 4 ký tự bắt đầu bằng 84 (VD `8471`). Bỏ trống → không constrain độ dài, khớp mọi độ dài.
...
**Section 4, 5, 6:** giữ nguyên v5 (Tháng, 4 cột, Logic 4-case).

### 5.5. Migration display value

Runner phải map cả code cũ (v5) VÀ display VN (v6):

```python
DISPLAY_TO_KEY = {
    # v6 VN
    "Bằng": "eq", "Trong danh sách": "in", "Trong khoảng": "between",
    "Bắt đầu bằng": "prefix", "Chứa": "contains", "Kết thúc bằng": "suffix",
    # v5 code (backward compat)
    "eq": "eq", "in": "in", "between": "between",
    "prefix": "prefix", "contains": "contains", "suffix": "suffix",
}
```
...
### 7.5. Parse operator VN + Digits

```python
def parse_column_sheet_v6(sheet, valid_cols, op_defaults, op_builder):
    filters = []
    select_cols = []
    warnings = []

    for row in sheet.iter_rows(min_row=2, max_col=5, values_only=True):
        col, op_raw, val, digits, out = [cell_text(c) for c in row]
        ...
        # Map VN → key
        op = DISPLAY_TO_KEY.get(op_raw, op_raw.lower()) if op_raw else ""

        has_op = op in op_builder.registry["operators"]
        has_val = bool(val)
        digits_int = int(digits) if digits and digits.isdigit() else None

        if op_raw and not has_op:
            raise RequestError(f"Cột {col}: toán tử không hợp lệ '{op_raw}'.")

        # Validate digits chỉ dùng cho supports_digits ops
        if digits_int is not None:
            if not has_op:
                warnings.append(f"Cột {col}: có Digits nhưng thiếu Toán tử. Digits bỏ qua.")
                digits_int = None
            elif not op_builder.registry["operators"][op]["supports_digits"]:
                warnings.append(f"Cột {col}: Toán tử '{op_raw}' không hỗ trợ Digits. Digits bỏ qua.")
                digits_int = None
            elif digits_int < len(val.split(",")[0].strip()):
                raise RequestError(f"Cột {col}: Digits ({digits_int}) < độ dài value ('{val}').")

        # 4 case (giữ logic v5, thêm digits)
        if has_op and has_val:
            op_builder.validate(col, op, val, digits_int)
            filters.append({"col": col, "op": op, "val": val, "digits": digits_int})
            if col not in select_cols:
                select_cols.append(col)
        elif has_op and not has_val:
            raise RequestError(f"Cột {col}: có Toán tử '{op_raw}' nhưng thiếu Giá trị.")
        elif not has_op and has_val:
            default_op = op_defaults.get(col)
            if default_op in op_builder.registry["operators"]:
                op_builder.validate(col, default_op, val, None)
                filters.append({"col": col, "op": default_op, "val": val, "digits": None})
                if col not in select_cols:
                    select_cols.append(col)
                warnings.append(f"Cột {col}: auto {default_op} (user không chọn toán tử)")
            else:
                warnings.append(f"Cột {col}: có Giá trị nhưng thiếu Toán tử và không có default. Bỏ qua.")
        else:
            if out.upper() == "YES" and col not in select_cols:
                select_cols.append(col)
...
- `request_template.xlsx` — regen v6 với 5 sheet + dropdown VN + cột Digits
...
11. Sau 1-2 tuần chạy ổn → xoá `docs/SPEC_v5_archived.md`, `inbox/` folder cũ

### 8.3. Backward compat

- File request v5 (không có cột Digits, dùng code text `eq/in/...`) — runner v6 vẫn parse được (map qua `DISPLAY_TO_KEY`), warning "template v5, download v6"
...
- **T33** — `builder.build_where("ma_so", "prefix", "84,85", digits=4)` → SQL `(ma_so LIKE ANY(...)) AND LENGTH(ma_so) = 4`, params đúng.
- **T34** — `builder.build_where("ma_so", "suffix", "AA", digits=6)` → SQL `ma_so LIKE '%AA' AND LENGTH(ma_so) = 6`.
...
- **T43** — Sheet Cột Export có 5 cột header (thêm Digits).
- **T44** — Data validation Toán tử = dropdown VN 6 options.
- **T45** — Cột Digits conditional formatting: row có Toán tử = "Chứa" → cell Digits xám.
...
- **T48** — Parse row `Toán tử="Bắt đầu bằng"` → op=`prefix`.
- **T49** — Parse row `Toán tử="prefix"` (v5 code) → op=`prefix` (backward compat).
- **T50** — Parse row `Toán tử="Bằng", Digits=4` → warning "eq không hỗ trợ Digits", digits bỏ qua.
- **T51** — Parse row `Toán tử="Bắt đầu bằng", Val="84", Digits=3` → RequestError "Digits < độ dài val".
...
- **T67** — Cardinality scan → make-template → generated template có dropdown VN + dropdown value cho cột cardinality thấp.
...
- Sheet Cột Export/Import thêm cột Digits + dropdown VN
...
- `parse_column_sheet_v6` với 5 cột, map VN, validate Digits
...
- [ ] Mở template trong Excel: dropdown Toán tử là tiếng Việt 6 options
- [ ] Cột Digits: chọn "Bắt đầu bằng" → cell active; chọn "Bằng" → cell xám
- [ ] Cột `ma_nuoc` có dropdown value (nếu cardinality ≤ threshold)
- [ ] Bảng=both → output 2 sheet đúng
...
- [ ] All 99 tests pass (61 v5 + 38 v6)
- [ ] README v6 update
- [ ] `docs/SPEC.md` archive thành `docs/SPEC_v5_archived.md`
